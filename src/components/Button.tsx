...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    // Variant classes using design token colors
    ...
    // Size classes using design token spacing
    const sizeClasses = {
    ...
    const leftIconClasses = cn(
      ["xs", "sm", "md"].includes(size) && "mr-2",
      ["lg", "xl"].includes(size) && "mr-3"
    );

    const rightIconClasses = cn(
      ["xs", "sm", "md"].includes(size) && "ml-2",
      ["lg", "xl"].includes(size) && "ml-3"
    );
